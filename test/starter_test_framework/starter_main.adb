...
procedure Starter_Main is
  ...
  procedure Search_Directory(Inp_Path: String;Out_Dir: String) is
      ...
      begin
            ...
            if Row(1) = "true" then
               test_start_button := True;
            elsif Row(1) = "false" then
               test_start_button := False;
            else
               raise Constraint_Error with "Invalid value for test_alarm_button: " & To_String(Row(3));
            end if;

            test_starter_state.state := S_State'Val(Integer'Value(To_String(Row(2))));

            test_speed := Integer'Value(To_String(Row(3)));
            ...
            Add_Output(Row, S_State'Enum_Rep(Out_Val)'Image);
